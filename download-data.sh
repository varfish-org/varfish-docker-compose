...
export V_ANNONARS_DATA_CLINVAR_CLINVAR=${V_ANNONARS_DATA_CLINVAR_CLINVAR-20240528}
# annonars-data-clinvar annonars
export V_ANNONARS_DATA_CLINVAR_ANNONARS=${V_ANNONARS_DATA_CLINVAR_ANNONARS-0.37.0}
# dotty-seqrepo
...
EOF
    # Possibly limit download list
    if [[ -z "$LIST_GREP" ]]; then
        cp /tmp/download-list.raw.txt /tmp/download-list.txt
    else
        grep "$LIST_GREP" /tmp/download-list.raw.txt \
        > /tmp/download-list.txt
    fi
